...
{
	int currAngle = SensorValue[liftPoten];
	while(currAngle > 400) //Lift to drop pos - FIX NUMBER
	{
		setLiftPower(127);
...
void preloadLaunch()
{
	...
	driveWithPincerCont(-500, 127, 25, 1500);
	setPincerPower(-127);
	setLeftDrivePower(0);
	setRightDrivePower(0);
	launch();
...
void runProgSkills(string side)
{
	...
	driveForDistance(-60); //drive up a little to give space
...
void runCompAutonAngled(string side)
{
	clearTimer(T1);
