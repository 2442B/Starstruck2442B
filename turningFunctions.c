...
void turn(const long angle)
{
	...
	long encoderLeft = SensorValue[leftEncoder], encoderRight = SensorValue[rightEncoder];
	...
	while (!atTarget)
	{
		...
		setLeftDrivePower(angleOutput);
		setRightDrivePower(-1 * angleOutput);
...
//Uses semaphores, skipping for now
/*
/**
